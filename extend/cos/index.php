...
class CosTencent
{
    ...
    {
        self::$bucket = env("tencent.bucket");
        $secretId = env("tencent.id"); //"云 API 密钥 SecretId";
        $secretKey = env("tencent.key"); //"云 API 密钥 SecretKey";
        $region = env("tencent.region"); //设置一个默认的存储桶地域
    ...
    public static function upload($name, $path)
    {
        $cosClient = self::init();
        try {
            $bucket = env("tencent.bucket"); //存储桶名称 格式：BucketName-APPID
